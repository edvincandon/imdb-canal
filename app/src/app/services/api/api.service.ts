...
const buildSearchQuery = (title: string) => `query { getByTitle(title:"${title}") { id primaryTitle originalTitle startYear genres } }`
const buildMutation = ({ id, primaryTitle, originalTitle, startYear, genres }: Movie ) => `mutation { update(id: ${id}, input: { primaryTitle: ${primaryTitle} originalTitle: ${originalTitle} startYear: ${startYear} genres: ${genres} }) { id primaryTitle originalTitle startYear genres } }`
